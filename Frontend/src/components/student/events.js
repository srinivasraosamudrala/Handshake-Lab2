...
import { getEventDetails } from "../../redux/actions/index";
...
class Events extends Component {
    ...
    registerEvent = (eventId, companyId, eligibility) => {
        ...
        if (this.props.studentedu.length) {
            if ((eligibility !== this.props.studentedu) && (eligibility !== 'All Majors')) {
                this.setState({
                    showalert: true
                })
            } else {
    ...
    render() {
        ...
        console.log(this.props.studentedu)
        ...
        if (this.props.eventlist) {
            ...
            if (this.state.showupdatealert === true) {
                erroralert = alert("Update your education details inorder to register for the event")
                this.setState({
                    showupdatealert : false
                })
            }
            ...
            if (eventlist.length > 0) {
                ...
                events = (
                    <div>
                        <div style={{ height: '500px', overflow: 'auto' }}>
                            <div style={{ position:'relative',fontWeight:'700', top:'5px',marginBottom:'16px'}}>Events</div>
                            ...
                            {eventlist.slice(this.state.page * this.state.rowsPerPage, this.state.page * this.state.rowsPerPage + this.state.rowsPerPage).map((event, index) => {
                                return (<div >
                                    <Link onClick={() => this.showEvent(((this.state.page)*(this.state.rowsPerPage))+index)} style={{ color: 'rgba(0, 0, 0, 0.8)' }}>
                                        <div class="col-md-2" style={{ marginTop: '10px', position: 'relative', left: '10px' }}><Avatar src={event.Company[0].image ? event.Company[0].image : this.state.emptyprofilepic} style={{ height: '50px', width: '50px', borderRadius: '0px', position: 'relative', left: '-20px' }} >DP</Avatar></div>
                ...
                if (eventdetailed.registrations) {
...
const mapStateToProps = state => {
    console.log(state)
    return {
        eventlist: state.studenteventlist,
        studentedu: state.studentmajor
    };
};

function mapDispatchToProps(dispatch) {
    return {
        getEventDetails: payload => dispatch(getEventDetails(payload)),
    };
}
